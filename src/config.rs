...
pub fn get_config() -> Config {
    ...
    let ssid: String = format!("HalleyHub-{}", env::var("BALENA_DEVICE_UUID").ok().to_string()[0..12]);

    let passphrase: Option<String> = Some(env::var("PAIRING_CODE").ok().to_string().pad(8, '_', Alignment::Right, false));
